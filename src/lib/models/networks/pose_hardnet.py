...
class HarDNetSeg(nn.Module):
    def __init__(self, num_layers, heads, pretrained, down_ratio, final_kernel,
        ...
        self.basew = HarDNetBase(num_layers).base
        ...
        if num_layers == 85:
          ...
          if pretrained:
            weights = torch.load('../models/hardnet85_base.pth')
            self.basew.load_state_dict(weights)
    ...
    def forward(self, x):
        ...
        for i in range(len(self.base)):
            x = self.base[i](x)
            if i in self.skip_nodes: # [1,3,8,13]
                xs.append(x)
        
        x = self.last_proj(x) # ConvLayer(784, 256, kernel=1)
        x = self.last_pool(x) # nn.AvgPool2d(kernel_size=2, stride=2)
        x2 = self.avg9x9(x) # nn.AvgPool2d(kernel_size=(9,9), stride=1, padding=(4,4))
        x3 = x/(x.sum((2,3),keepdim=True) + 0.1)
        x = torch.cat([x,x2,x3],1)
        x = self.last_blk(x) # self.last_blk = HarDBlock(768, 80, 1.7, 8)
        ...
        #z['wh'] = F.relu(z['wh'])
        #feature_loc = self.locations(z['wh'])[None]
        #pred_bboxes = self.apply_ltrb(feature_loc, z['wh'])
        #z['bbox'] = pred_bboxes
        return [z]
    

def get_pose_net(num_layers, heads, head_conv=128, down_ratio=4, trt=False):
    model = HarDNetSeg(
                 num_layers,
                 heads,
                 pretrained=True,
